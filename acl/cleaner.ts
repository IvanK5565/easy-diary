...
import { IRoles, AclRole, IRules } from "./types";
import Acl from "./Acl";
import BaseContext from "@/server/di/BaseContext";

export class Cleaner extends BaseContext{
    public guestRolesAndRules(){
        const acl = new Acl(this.ctx.roles, this.ctx.rules);
        return this.cleanRolesAndRules(acl, AclRole.GUEST);
    }
    public cleanRolesAndRules(acl:Acl, role:AclRole){
        return {
            roles:this.cleanRoles(acl,role),
            rules:this.cleanRules(acl,role),
        }
    }
    public cleanRoles(acl:Acl, role: AclRole): IRoles {
        const { roles } = this.ctx;
        return Cleaner.cleanRoles(acl, roles, role);
    }
    public cleanRules(acl:Acl, role: AclRole): IRules {
        const { rules } = this.ctx;
        return Cleaner.cleanRules(acl, rules, role);
    }
    public static cleanRoles(acl:Acl, roles:IRoles, role:AclRole){
        const result: IRoles = {};
    ...
    public static cleanRules(acl:Acl, rules:IRules, role:AclRole){
        const result: IRules = {};
...
// export function cleanRoles(guard: Guard, role: AclRole): IRoles {
//     const result: IRoles = {};
...
// export function cleanRules(guard: Guard, role: AclRole): IRules {
//     const result: IRules = {};
