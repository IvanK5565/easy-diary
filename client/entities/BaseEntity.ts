...
import { IPagerParams, TPaginationInfo } from "../store/types";
...
export default abstract class BaseEntity extends BaseContext {
  ...
  public get actions() {
    ...
    return Object.fromEntries(
      actions.map((action) => [
        action,
        (data?: any) => ({ type: action, payload: data }),
      ]),
    );
  }

  public static sagas(di: IClientContainer) {
    const actions: (keyof IEntityContainer)[] =
  ...
  private static async xFetch(
    ...
    const sBody = body ? JSON.stringify(body) : "{}";
    const query =
      body && method === "GET"
        ? `?${new URLSearchParams(JSON.parse(sBody)).toString()}`
        : "";
    const res: Response = await fetch(`/api/${url}${query}`, {
      method,
      body: body && method === "POST" ? sBody : undefined,
    }).then((data) => data.json());
    if (!res.success) {
      throw new XFetchError(`${res.message}; body:${body}`);
    } else {
      return res;
    }
  }
