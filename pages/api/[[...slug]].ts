...
function findBestMatch(routes: [Route, string][], path: string) {
  ...
  for (let i = 1; i < filtered.length; i++) {
    ...
    if (comparingValue > 0) {
      ...
      console.warn(
        "The same routes matching the path finded:",
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const preparedRoutes = routesData.map(
    ([route, controller]) => [new Route(route), controller] as [Route, string],
  );
  ...
  if (finded && container.hasRegistration(finded[1])) {
    ...
    const route = finded[0];
    console.error(
      "Controller not found for route: ",
      route.toString(),
      " but route matched.",
    );
  } else {
