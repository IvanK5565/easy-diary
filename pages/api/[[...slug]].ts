...
function findBestMatch(routes: [Route, string][], path: string) {
	...
	for (let i = 1; i < filtered.length; i++) {
		...
		if (comparingValue > 0) {
			...
			console.warn('The same routes matching the path finded:',
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  console.log('routes:', routesData)
	console.log('Finded route', finded?.[0].toString(), 'for path', path)
	...
	if (finded && container.hasRegistration(finded[1])) {
		...
		console.info("Requesting controller: ", name, " for route: ", route.toString(), " with path: ", path);
		controller.handler(route.toString())(req, res);
	} else if (finded) {
		const route = finded[0];
		console.error("Controller not found for route: ", route.toString(), " but route matched.");
	} else {
		// res.writeHead(301, {
		// 	Location: "/notFound",
		// });
