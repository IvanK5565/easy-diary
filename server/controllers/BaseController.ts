...
export default abstract class BaseController extends BaseContext {
  ...
  private createAction(handler: string, route: string): ActionAdapter {
    return (req, res) => {
      ...
      return (
        this.getActionProps(req, res, route)
          .then((props) => fn(props))
          // .then(r => {
  ...
  private async getActionProps(
    ...
    await this.ctx.mutex.runCritical("getActionProps", async () => {
      if (!req.session) {
        console.log(
          "get session in getActionProps for route:",
          route,
          "in controller:",
        ...
        req.session = await getServerSession<AuthOptions, Session>(
          req,
          res,
          this.ctx.authOptions,
        );
      } else {
        console.log(
          "session already exists in getActionProps for route:",
          route,
          "in controller:",
          this.constructor.name,
        );
      ...
      if (!req.guard) {
        console.log(
          "create Guard in getActionProps for route:",
          route,
          "in controller:",
          this.constructor.name,
        );
        const { roles, rules, role } = req.session?.identity || GUEST_IDENTITY;
        req.guard = new Guard(roles, rules, role);
      } else {
        console.log(
          "Guard already exists in getActionProps for route:",
          route,
          "in controller:",
          this.constructor.name,
        );
